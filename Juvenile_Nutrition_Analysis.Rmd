...
survplotKM4$plot <- survplotKM4$plot+ 
              ggplot2::annotate("text", 
                                x = 15, y = .35, # x and y coordinates of the text
                                label = "p(Nutrition)=0.473", size = 7, color="darkgray")+
              ggplot2::annotate("text", 
                                x = 15, y = .25, # x and y coordinates of the text
                                label = "p(Nutrition x Temperature)=0.086", size = 7, color="darkgray")
survplotKM4
...
Growth1a<-ggplot(data=GrowthPlanar, aes(x=Temperature, y=mean, colour=Nutrition)) +
  ...
  xlab("Temperature") +
  geom_text(x=1.5, y=0.9, label="p(Nutrition)=0.035", size=6, color="black") + 
 ...
 #scale_y_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1))+
  ...
  annotate("text", x = .45, y = 15, 
   ...
   annotate("text", x = .45, y = -01, 
           label = "p(Stress Temp. x Nutrition)=0.005", size = 5, color="black", hjust=0)+
   annotate("text", x = .45, y = -05, 
           label = "p(Rearing Temp. x Nutrition)=0.009", size = 5, color="black", hjust=0)+
