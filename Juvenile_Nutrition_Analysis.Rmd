...
RearingDay<-ggplot(Rearing1, aes(x = Minutes, y = mean, color=Treatment, fill=Treatment)) + 
  ...
  theme(plot.margin = unit(c(1, 0.5, 0.1, 1), "cm")) + #t, r, b, l
  theme(axis.title = element_text(size = 16, color="black", face="bold"))+
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 5, b = 0, l = 0))) +
  theme(axis.title.x = element_text(margin = margin(t = 5, r = 0, b = 0, l = 0))) +
  theme(axis.text = element_text(size=16, color="black"))
...
ggsave(filename="Figures/TemperatureFigure.png", plot=tempplots, dpi=300, width=13, height=6, units="in")
```
...
survplotKM4$plot <- survplotKM4$plot+ 
              ggplot2::annotate("text", 
                                x = 15, y = .35, # x and y coordinates of the text
                                label = "p(Feeding)=0.473", size = 7, color="darkgray")+
              ggplot2::annotate("text", 
                                x = 15, y = .25, # x and y coordinates of the text
                                label = "p(Feeding x Temperature)=0.086", size = 7, color="darkgray")
survplotKM4
...
Growth1a<-ggplot(data=GrowthPlanar, aes(x=Temperature, y=mean, colour=Nutrition)) +
  ...
  xlab("Temperature") +
  geom_text(x=1.5, y=0.9, label="p(Feeding)=0.035", size=6, color="black") + 
  geom_text(x=1.5, y=0.6, label="p(Temperature)=0.824", size=6, color="darkgray") + 
  geom_text(x=1.5, y=0.3, label="p(Interaction)=0.591", size=6, color="darkgray")
...
Thick2<-ggplot(data=ThickTable, aes(x=Temperature, y=mean, colour=Nutrition)) +
  ...
  xlab("Temperature") +
  scale_x_discrete(limits=c("Cool", "Ambient"))+
  geom_text(x=1.5, y=410, label="p(Feeding)<0.001", size=6, color="black") + 
  geom_text(x=1.5, y=375, label="p(Temperature)=0.060", size=6, color="darkgray") + 
  geom_text(x=1.5, y=340, label="p(Interaction)=0.037", size=6, color="black")
...
Red2<-ggplot(data=RedTable, aes(x=Temperature, y=mean, colour=Nutrition)) +
  scale_colour_manual(name="Feeding",values=c("orange", "purple"), labels=c("Fed", "Unfed"))+
  geom_line(position=position_dodge(0.03), size=1.3, aes(group=Nutrition)) +
  geom_point(size=3, position=position_dodge(0.03)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.03), size=1.3) +
  geom_text(x=1.5, y=21, label="p(Feeding)<0.001", size=6, color="black") + 
  geom_text(x=1.5, y=19, label="p(Temperature)=0.858", size=6, color="darkgray") + 
...
FigSurvStress <- ggplot(data=Stress.Summary, aes(x=factor(Stress.Temp), y=mean, group=interaction(Rear.Temp, Nutrition), colour=Rear.Temp, linetype=Nutrition)) + 
  ...
  geom_point(size=3, position=position_dodge(0.03)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.03), size=1.3) +
  scale_x_discrete(labels=c("Ambient" = "Ambient (27.4°C)", "High" = "High (30.1°C)"))+
 ...
 #scale_y_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1))+
  scale_colour_manual(name=expression(bold(atop("Rearing         ", paste("Temperature")))),
                        ...
                        values=cols)+
  scale_linetype_manual(name=expression(bold("Feeding")),
  ...
  annotate("text", x = .45, y = 15, 
   ...
   annotate("text", x = .45, y = 07, 
           label = "p(Feeding)<0.001", size = 5, color="black", hjust=0)+
   annotate("text", x = .45, y = 03, 
           label = "p(Stress Temp. x Rearing Temp.)=0.658", size = 5, color="darkgray", hjust=0)+
   annotate("text", x = .45, y = -01, 
           label = "p(Stress Temp. x Feeding)=0.005", size = 5, color="black", hjust=0)+
   annotate("text", x = .45, y = -05, 
           label = "p(Rearing Temp. x Feeding)=0.009", size = 5, color="black", hjust=0)+
   annotate("text", x = .45, y = -09, 
           label = "p(Three-way Interaction)=0.736", size = 5, color="darkgray", hjust=0)+
...
Plot total survivorship in high and ambient treatment at the end of the stress period with nutrition color scheme.    
 
```{r, results=TRUE}
#cols <- c("Ambient" = "#606060","Cool" = "#0099FF")
          
FigSurvStress <- ggplot(data=Stress.Summary, aes(x=factor(Stress.Temp), y=mean, group=interaction(Rear.Temp, Nutrition), colour=Nutrition, linetype=Rear.Temp)) + 
  geom_line(position=position_dodge(0.03), size=1.3) +
  geom_point(size=3, position=position_dodge(0.03)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.03), size=1.3) +
  scale_x_discrete(labels=c("Ambient" = "Ambient (27.4°C)", "High" = "High (30.1°C)"))+
  ...
  theme_classic() +
 #scale_y_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1))+
  scale_linetype_manual(name=expression(bold(atop("Rearing         ", paste("Temperature")))),
                        breaks=c("Ambient", "Cool"),
                        labels=c("Ambient", "Cool"), 
                        values=c("solid", "dotted"))+
  scale_color_manual(name=expression(bold("Feeding")),
                          breaks=c("Heterotrophy", "No Heterotrophy"),
                          labels=c("Fed", "Unfed"), 
                          values=c("orange", "purple")) +
  annotate("text", x = .45, y = 15, 
