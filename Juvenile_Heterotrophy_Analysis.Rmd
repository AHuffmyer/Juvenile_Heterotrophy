---
title: Effects of nutritional and thermal regimes on juvenile coral physiology and thermal tolerance  
...
TempsJune%>%
       summarise(Junemean_temp=mean(Junemean), Junemax_temp=max(Junemean), Junemin_temp=min(Junemean), Junemean_range=mean(Junerange), Junemax_range=max(Junerange), Junemin_range=min(Junerange))
...
TempsMarch%>%
       summarise(Marchmean_temp=mean(Marchmean), Marchmax_temp=max(Marchmean), Marchmin_temp=min(Marchmean), Marchmean_range=mean(Marchrange), Marchmax_range=max(Marchrange), Marchmin_range=min(Marchrange))
```
...
There were 62.14 Degree Heating Days in the high treatment as compared to the mean of the *ambient treatment* over the course of the 31-day stress period.  
```{r, results=TRUE}
...
Growth1b<-ggplot(data=GrowthPlanar, aes(x=Temperature, y=mean, fill=Nutrition)) +
  ...
  geom_text(x=1.5, y=4.3, label="p(Interaction)=0.59", size=6, color="darkgray");Growth1b
...
Growth3<-ggplot(data=growth, aes(x=Temperature, y=graph, fill=Nutrition)) +
  ...
  #geom_text(x=2.23, y=26.5, label="A", size=8)+
  xlab(expression(bold(paste("Temperature"))));Growth3
...
Growth3QT<-ggplot(data=growth, aes(x=Temperature, y=tPlanar, fill=Nutrition)) +
  ...
  #geom_text(x=2.23, y=0.8, label="A", size=8)
...
ggsave("Figures/Thickness_Figure.pdf", plot=Thick3, height=8, width=6, units = c("in"), dpi=300) #output figure
```
...
Red3<-ggplot(data=colony, aes(x=Temperature, y=Red, fill=Nutrition)) +
  ...
  xlab("Temperature");Red3

ggsave("Figures/Fluorescence_Figure.pdf", plot=Red3, height=8, width=8, units = c("in"), dpi=300) #output figure
```
...
Analyze survivorship during thermal stress using a binomial mixed effects model.  

Main effects: Day, pre-stress temperature, nutrition, and stress temperature  
Random effects: Plug nested within tank and cohort  
...
```{r, results=TRUE}
dispersion_glmer(stressmod) #no evidence of overdispersion
```
...
stress.effects2<-plot(eff.stress2,
                   lines=list(multiline=TRUE, 
                              ...
                              lty=c(1, 2, 1, 2)), 
                   confint=list(style="bands", alpha=0.0), 
                   lwd=6,
                   ...
                   lattice=list(key.args=list(space="right",
                                              ...
                                              cex=1.5,
                                              cex.title=1.5,
                                              title=expression(bold("Treatments")))))
```
...
stress.effects3<-plot(eff.stress3,
                   lines=list(multiline=TRUE, 
                              ...
                              lty=c(1, 2, 1, 2)), 
                   confint=list(style="bands", alpha=0.00), 
                   lwd=6,
                   axes=list(y=list(lim=c(0, 1))),
                   type="response", 
                   ylab=expression(bold("Survivorship")), 
...
stress_figs_30days<-plot_grid(stress.effects3, stress.effects2, labels = c("Control", "High"), ncol=2, nrow=1, rel_heights= c(1,1), rel_widths = c(0.75,1), label_size = 20, label_y=1, align="h")

ggsave(filename="Figures/Stress_Survival_OverTime.png", plot=stress_figs_30days, dpi=300, width=20, height=8, units="in")

detach(thermal)
```

![Stress survival over 30-day period in control and high temperatures.](Figures/Stress_Survival_OverTime.png)   
 
Summary of total survivorship included below at the end of the thermal stress test.   
...
Plot results of binomial model of survivorship at the end of the stress period.  

```{r, results=TRUE}
final$group<-paste(final$Rear.Temp, "-", final$Nutrition)

stressFINALmod2<-glmer(x ~ group * Stress.Temp + (1|Stress.Tank) + (1|Cohort), family=binomial, data=final) 

eff.stressFINAL <- predictorEffect(c("Stress.Temp"), stressFINALmod2)

stress.effectsFINAL<-plot(eff.stressFINAL,
                   lines=list(multiline=TRUE, 
                              col=c("gray", "gray", "blue", "blue"), 
                              lty=c(1, 2, 1, 4)), 
                   confint=list(style="none", width=0), 
                   lwd=6,
                   axes=list(y=list(lim=c(0, 1))),
                   type="response",
                   ylab=expression(bold("Survivorship")), 
                   legend.position="right",
                   main="",
                   xlab=expression(bold("Stress Treatment")),
                   lattice=list(key.args=list(space="top",
                                              border=FALSE, 
                                              cex=1.2,
                                              cex.title=1.2,
                                              title=expression(bold("Pre-Stress Temperature - Nutrition")))))

stress.effectsFINAL<-ggdraw(stress.effectsFINAL) +
              ggplot2::annotate("text", 
                                x = 0.5, y = .75, # x and y coordinates of the text
                                label = "p(Pre-Stress Temperature x Nutrition)=0.008", size = 6, color="black")+
              ggplot2::annotate("text", 
                                x = 0.5, y = .7, # x and y coordinates of the text
                                label = "p(Stress Temperature x Nutrition)=0.004", size = 6, color="black")

stress_figs_end<-plot_grid(stress.effectsFINAL, labels = c(""), ncol=1, nrow=1, rel_heights= c(1), rel_widths = c(1), label_size = 20, label_y=1, align="h")

ggsave(filename="Figures/Stress_Survival_Total.png", plot=stress_figs_end, dpi=300, width=7, height=10, units="in")
          
```


![Stress survival over 30-day period in control and high temperatures.](Figures/Stress_Survival_Total.png)   