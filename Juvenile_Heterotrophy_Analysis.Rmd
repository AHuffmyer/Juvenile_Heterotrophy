...
```{r setup, echo=TRUE}
knitr::opts_chunk$set(root.dir = "~/Juvenile_Heterotrophy",warning=FALSE, message=FALSE)
```
...
lmerTest::anova(modelGrowth1, type=2)
```
...
write.csv(confocal, file="Data/Master_Confocal.csv")
```
...
To view response metrics (tissue thickness and symbiont fluoresence) and their covariate (surface area) in relation to each other, we construct a Principle Components Analysis and analyze the groupings by treatments with a PERMANOVA in the vegan package.  

First, construct a PCA at the colony level with tissue thickness, symbiont fluorescence, and surface area.  Plot indicates the explained variance of each PCA component.  
```{r, results=TRUE}
pca<-colony[c("Sample", "Thickness", "Temperature", "Nutrition", "Area", "Red", "PolypThickness", "PolypRed", "ConnectThickness", "ConnectRed")]
#assign binary values to temperature = 1 is cool, 2 is ambient, assign binary values to nutrition = 1 is hetero, 2 is no hetero
pca$Treatment<-paste(pca$Temperature, pca$Nutrition)
pca$Ambient[pca$Temperature=="Cool"] <- 1
pca$Ambient[pca$Temperature=="Ambient"] <- 2
pca$NoHeterotrophy[pca$Nutrition=="No Heterotrophy"] <- 2
pca$NoHeterotrophy[pca$Nutrition=="Heterotrophy"] <- 1

pca<-pca[complete.cases(pca),]

colony.pca<-prcomp(pca[c(2,6,5)],scale=TRUE, center=TRUE) #12,13
summary(colony.pca)
...
```{r,results=TRUE}
eig.val <- get_eigenvalue(colony.pca)
eig.val

res.var <- get_pca_var(colony.pca)
...
```{r, results=FALSE}
pcaPlot1<-ggplot2::autoplot(colony.pca, data=pca, frame.colour="Treatment", loadings=TRUE,  colour="Treatment", loadings.label.colour="black", loadings.colour="black", loadings.label=FALSE, frame=TRUE, frame.type="norm", loadings.label.size=5, loadings.label.vjust=-1) + 
  scale_colour_manual(values=c("#606060","#E0E0E0", "#0066CC","#99CCFF")) +
  scale_fill_manual(values=c("#606060","#E0E0E0", "#0066CC","#99CCFF")) + 
  xlim(-0.4,0.4) +
  #geom_text(x=0.16, y=0.08, label="No Heterotrophy", size=6) +
  geom_text(x=-0.07, y=0.21, label="Surface Area", size=6) +
  geom_text(x=-0.15, y=0.06, label="Tissue Thickness", size=6) +
  geom_text(x=-0.14, y=-0.17, label="Symbiont Fluorescence", size=6) +
  #geom_text(x=0.11, y=-0.15, label="Ambient Temperature", size=6) +
  theme(legend.text = element_text(size=18), 
...
ggsave("Figures/PCA.pdf", plot=pcaPlot1, height=8, width=12, units = c("in"), dpi=300) #output figure
```

Generate final figure of response variables including PCA.    
```{r, results=FALSE}
confocalfig1<-plot_grid(Thick3, Red3, pcaPlot1, labels = c("A", "B", "C"), ncol=3, nrow=1, rel_heights= c(1,1,1), rel_widths = c(0.8,1.25 ,1.5), label_size = 20, label_y=1, align="h")

ggsave(filename="Figures/confocalfig1.png", plot=confocalfig1, dpi=500, width=20, height=6, units="in")

photo2 <- cowplot::ggdraw() + cowplot::draw_image("Figures/Color_corrected.jpg", scale = 1)
...
Finally, conduct PERMANOVA with vegan package.  

```{r, results=FALSE}
require(vegan)

# PCA
data_vegan <- pca[c(2,5,6)]
vegan_colony <- scale(data_vegan)
```

```{r, results=TRUE}
# PerMANOVA - partitioning the euclidean distance matrix by nutrition & temp
adonis1<-adonis2(vegan_colony ~ Nutrition * Temperature, data = pca, method='eu', perm=1000)
summary(adonis1)
adonis1
```
...
1. PERMANOVA test indicates there is a significant interactive effect of nutrition and temperature.  
2. PC1 primarily is driven by differences in tissue thickness.  
3. PC2 is driven by differences in surface area and symbiont fluorescence, which are negatively correlated with each other.  
4. In a multi-dimensional space, interactive effects are present, as seen in analysis of individual variables.  
