---
title: Juvenile *Pocillopora *acuta coral nutritional and thermal regime enhance resistance to temperature stress through physiological preparation
...
2. 24-hour cycle of temperatures by summarizing the mean temp at each 15-minute interval per day during the pre-stress period and the stress period. 
...
Display temperature means, standard deviation, and mean daily range for 1) each pre-stress treatment; 2) each stress treatment; 3) full assembled figure.  
...
Display descriptive statistics for pre-stress treatments.   
...
## 3. Post-Settlement Survivorship  

Survivorship was recorded weekly on all juvenile plugs in the experiment (n = ~715 plugs) as either 0 (at least one living spat) or 1 (no living spat) as required for analyses across multiple timepoints during the 30-day pre-stress treatment period. Data are examined as survivorship curves and analyzed using Cox Proportional Hazards models with temperature and nutrition as main effects.  

```{r, results=FALSE}
#load data
KM<-read.csv("Data/PreStress_Survivorship.csv", header=TRUE, sep=",", na.strings="NA") #load data
...
Summary of survivorship in table below for each temperature and nutrition treatment at each timepoint.   
...
2. Survivorship was significantly higher in the cool temperature treatment, but to a marginal degree.    


## 4. Growth    

Growth data is collected as planar surface area of each juvenile at the start and end of the pre-stress period (30 days).  
...
Growth rate is calculated as the change in planar surface area normalized to starting size per day (% planar extension per day).   
...
```{r, results=TRUE}
modelGrowth=lmer(scale(tPlanar)~Nutrition * Temperature + (1|Fused.Initial) + (1|Tank/Plug.ID) + (1|Cohort), data=growth)
```
...
Display figure of juvenile growth rate.  
...
*Conclusions*  
1. Growth was significantly higher in heterotrophic feeding treatment. 

2. There was no effect of temperature.  
...
Examine data for normality, using data collected as averaged over each juvenile colony (including polyp and connecting tissue measurements).  
...
```{r, results=TRUE, message=FALSE, warning=FALSE}
RedModel<-lmer(scale(Red)~Temperature+Nutrition+Temperature:Nutrition + (1+Area|Plug) + (1|Cohort) + (1|Tank/Plug), data=colony)
summary(RedModel)
...
Survivorship was measured as the number of inidividual colonies alive on each plug at the beginning and end of the stress test. 
